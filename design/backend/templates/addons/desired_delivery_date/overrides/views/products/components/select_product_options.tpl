...
<div class="controls">
    ...
    {elseif $po.option_type == "D"} {*Date*}
        <div class="controls">
        <input id="option_{$id}_{$po.option_id}" type="date" name="{$name}[{$id}][product_options][{$po.option_id}]" value="{$po.value|default:$po.inner_hint}" {if $cp.exclude_from_calculate && !$product.aoc}disabled="disabled"{/if} class="input-date {if $po.inner_hint}cm-hint{/if}" {if $po.inner_hint}title="{$po.inner_hint}"{/if} {if $update_options}onchange="fn_change_options('{$c_obj|default:$id}', {$id}, '{$po.option_id}');"{/if} />
            {include file="common/calendar.tpl"|default:"" start_year=$settings.Company.company_start_year}
        </div>
